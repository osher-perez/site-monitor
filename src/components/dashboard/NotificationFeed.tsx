...
export const NotificationFeed = ({ messages = [] }: NotificationFeedProps) => {
  ...
  const getFeedStyles = (type: "info" | "warning" | "success") => {
    switch (type) {
      ...
      case "warning":
        return "bg-amber-50/70 border-amber-200 text-amber-800";
      case "info":
      default:
    ...
    <div className="space-y-2 text-right" dir="rtl">
      ...
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2">
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`p-3.5 rounded-xl border text-xs font-medium leading-relaxed shadow-2xs transition-all ${getFeedStyles(
              msg.type
            )}`}
          >
            {msg.text}
