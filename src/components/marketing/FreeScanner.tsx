      ...
      <div className="bg-linear-to-b from-blue-50/60 to-blue-50/10 border border-blue-100 rounded-3xl p-8 md:p-10 shadow-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 font-medium max-w-md mx-auto leading-relaxed">
            הזן כתובת אתר (URL) לקבלת דוח מיידי על סטטוס השרת, זמני תגובה, אבטחת Headers ותקינות פרוטוקול ה-SSL.
          </p>
        </div>
          ...
          <div className="mt-8 space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
              <div className="text-center p-2">
                ...
                <div className={`font-extrabold text-sm flex items-center justify-center gap-1.5 ${result.realtime.status === 'ONLINE' ? 'text-emerald-600' : 'text-red-600'}`}>
              ...
              <div className="text-center p-2">
                <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">זמן תגובה כולל</div>
                <div className="font-extrabold text-sm text-gray-900 font-mono">{result.realtime.speed}</div>
              </div>

              <div className="text-center p-2">
                <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">תעודת SSL</div>
                <div className={`font-extrabold text-sm ${result.realtime.sslStatus.includes('תקף') ? 'text-blue-600' : 'text-amber-600'}`}>
                  {result.realtime.sslStatus}
                </div>
              </div>

              <div className="text-center p-2">
                <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">ציון אבטחת שרת</div>
                <div className={`font-extrabold text-sm ${['A', 'B'].includes(result.realtime.securityRating) ? 'text-purple-600' : 'text-red-500'}`}>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-100">
                ...
                <div className="p-4 flex justify-between items-center text-sm">
                  <span className="text-gray-600 font-medium">כותרת הגנה X-Frame-Options (מניעת הזרקות קוד):</span>
                  <span className="font-bold font-mono">
                    {result.realtime.headersAnalysis['X-Frame-Options'] ? '🟢 קיימת (מאובטח)' : '🔴 חסרה (סיכון קליקג\'קינג)'}
                  </span>
                </div>

                {/* בדיקת HSTS */}
                <div className="p-4 flex justify-between items-center text-sm">
                  <span className="text-gray-600 font-medium">כותרת Strict-Transport-Security (אכיפת HTTPS):</span>
                  <span className="font-bold font-mono">
                    {result.realtime.headersAnalysis['Strict-Transport-Security'] ? '🟢 פעילה' : '🔴 חסרה'}
                  </span>
                </div>

                {/* שורה חסומה 1 - SLA */}
                <div className="p-4 flex justify-between items-center text-sm bg-striped bg-slate-50/50">
                  <span className="text-gray-400 font-medium">מדד יציבות היסטורי חודשי (Uptime SLA %):</span>
                ...
                <div className="p-4 flex justify-between items-center text-sm bg-striped bg-slate-50/50">
                  <span className="text-gray-400 font-medium">גרף תנודתיות זמני תגובה בשעות שיא:</span>
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md tracking-wide">
                ...
                <div className="p-4 flex justify-between items-center text-sm bg-striped bg-slate-50/50">
                  <span className="text-gray-400 font-medium">ניתוב התראות מיידי (WhatsApp, SMS, Telegram):</span>
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md tracking-wide">
            ...
            <div className="p-5 bg-amber-50/40 border border-amber-100 rounded-2xl text-center">
              <p className="text-xs text-amber-800 leading-relaxed font-medium max-w-2xl mx-auto">
                💡 <strong>שים לב:</strong> בדיקה זו מייצגת צילום מצב (Snapshot) של השנייה הנוכחית בלבד. קוד משתנה, שרתים חווים עומסי פתע ותעודות SSL פוקעות בלילה. כדי להבטיח שאתה יודע על תקלות לפני הלקוחות שלך, מומלץ להפעיל ניטור רציף.
              </p>
            </div>
